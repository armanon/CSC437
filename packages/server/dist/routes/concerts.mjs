...
router.get("/", (req, res) => __async(void 0, null, function* () {
  try {
    const concerts = yield getAllConcerts();
    res.json(concerts);
  } catch (err) {
...
router.get("/:id", (req, res) => __async(void 0, null, function* () {
  try {
    const concert = yield getConcertById(req.params.id);
    if (concert) {
      res.json(concert);
    } else {
...
router.post("/", (req, res) => __async(void 0, null, function* () {
  try {
    const newConcert = yield addConcert(req.body);
...
router.put("/:id", (req, res) => __async(void 0, null, function* () {
  try {
    const updatedConcert = yield updateConcert(req.params.id, req.body);
    if (updatedConcert) {
...
router.delete("/:id", (req, res) => __async(void 0, null, function* () {
  try {
    const result = yield deleteConcert(req.params.id);
    if (result) {
      res.status(204).send();
    } else {
      res.status(404).send("Concert not found");
